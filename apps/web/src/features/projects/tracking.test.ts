...
describe('tabCounts', () => {
  it('counts every bucket from the same scored list', () => {
    ...
    // Burning (critical), Late (late work), Stalled (shot but barely started).
...
describe('summary', () => {
  it('counts projects, the same as the tab each tile opens', () => {
    const s = summary(BOARD)
    const tabs = tabCounts(BOARD)
    expect(s).toEqual({
      attention: tabs.attention,
      overdue: tabs.overdue,
      data_missing: tabs.data_missing,
      pending_review: tabs.pending_review,
    })
